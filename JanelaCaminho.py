...
def janela_caminho():
    ...
    frame = tk.Frame(master=janela, width=250, height=396, background='#0059b3')
    frame.pack(side=RIGHT)

    label = tk.Label(frame, text="Digite o caminho do arquivo:", background='#0059b3')
    label.place(x=25, y=75)

    entry = tk.Entry(master=frame, width=40)
    entry.place(x=25, y=105)

    caminho = tk.Label(frame, text="*O campo caminho do arquivo é\n obrigatório.", background='#0059b3')
    caminho.place(x=25, y=135)

    checkbox = tk.Checkbutton(frame, text="Lembrar", background='#0059b3')
    ...
    btn_caminho = tk.Button(frame, text="CONFIRMAR", width=20, background='#A52A2A')
